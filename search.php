...
<div align="center">
	...
	$cat = isset($_REQUEST['c']) ? " AND category='".clean_search($_REQUEST['c']) . "'" : null;
    $statement = null;

    if ($search || $cat) {
		$order_by = " ORDER BY added DESC";
        if (preg_match('/[a-f0-9]{40}/i', $search)) {
            $statement = $mysqli->prepare('SELECT * FROM torrents WHERE hash=?' . $cat . $order_by);
            $statement->bind_param('s', $search);
        } else {
            $conditions = advSearch($search);
            // $conditions = 'title = ?'; $s = "%$search%";
            $statement = $mysqli->prepare('SELECT * FROM torrents WHERE ' . $conditions . $cat . $order_by);
            // $statement->bind_param('s', $s);
        }
        $statement->execute();
        $statement->bind_result($id, $title, $hash, $age, $size, $category, $link);
    } else {
        header('location: /');
    }
        ...
        <table class="data" style="width: 100%" cellpadding="0" cellspacing="0">
            ...
            while ($statement !== null && $statement->fetch()): ?>
                ...
                <tr class="<?= $i % 2 == 0 ? 'even' : 'odd' ?>">
                    <td><a class="cellMainLink" href="<?= 'https://worldwidetorrents.eu/torrents-details.php?id=' . $link ?>"><?= $title ?></a></td>
